class ZeroOneKnapsack {
    ...
    private fun zeroOneKnapsackMemo(i:Int,bagCapacity: Int, weight: IntArray, prices: IntArray, dp: Array<IntArray>): Int {
        ...
        if (weight[i-1] <= bagCapacity) {
            val include = zeroOneKnapsackMemo(i-1, bagCapacity-weight[i-1], weight, prices, dp) + prices[i-1]
    ...
    fun knapsackByTabulation(weight: IntArray, prices: IntArray, bagCapacity: Int): Int {
        ...
        for (i in 1 .. weight.size) {
            for (j in 1 .. bagCapacity) {
                if (weight[i-1] <= j) {
                    val include = dp[i-1][j-weight[i-1]] +  prices[i-1]
...
fun main() {
    ZeroOneKnapsack().apply {
        ...
        println(knapsackByMemorization(weight = weight, prices = prices, bagCapacity = bagCapacity))  //75

        println(knapsackByTabulation(weight = weight, prices = prices, bagCapacity = bagCapacity))  //75
    }
}
