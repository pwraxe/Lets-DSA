...
class Dijkstra<T>(private val graphImplByAdjList: GraphImplByAdjList<T>) {
    ...
    fun letsFindShortestPathToAllVertex (startVertex: Vertex<T>)  : HashMap<Vertex<T>, Visit<T>> {
        ...
        val distComparator = Comparator<Vertex<T>> { first, second ->
            val firstTD = getShortPathToDestination(first, paths) .sumOf { it.weight ?: 0.0 }
            val secondTD = getShortPathToDestination(second, paths).sumOf { it.weight ?: 0.0 }                     //totalDistance(second, paths)
        ...
        while (true) {
            ...
            graphImplByAdjList.getAllEdges(vertex).forEach {
                val weight = it.weight ?: return@forEach

                val newDistance = getShortPathToDestination(vertex, paths).sumOf { it.weight ?: 0.0 } + weight
                val existingDistance = getShortPathToDestination(it.destinationVertex, paths).sumOf { it.weight ?: 0.0 }

                if(paths[it.destinationVertex] == null || newDistance < existingDistance) {
    ...
    fun getShortPathToDestination(destinationVertex: Vertex<T>, path: HashMap<Vertex<T>, Visit<T>>) : ArrayList<Edge<T>> {
...
fun main () {
    ...
    GraphImplByAdjList<String>().apply {
        ...
        val shortPath = dijktras.letsFindShortestPathToAllVertex(vertexA)

        //Step 1 : Get Graph converted to short path to all destination
//        shortPath.forEach { vertex, visit ->
//            println("${vertex.dataItem} === ${visit.visitType} ==> ${visit.edge}")
//        }

        //Step 2 : Get Short Path to Destination
        val path = dijktras.getShortPathToDestination(vertexF,shortPath)
        path.forEach {
